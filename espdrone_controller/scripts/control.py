...
# send roll, pitch, yaw data to PID() for attitude-stabilisation, along with 'f', to obtain 'fUpdated'
    ...
    (fr, fl, bl, br, err_roll, err_pitch, err_yaw) = PID(
        roll, pitch, yaw, f, args[7])
...
if __name__ == '__main__':
    try:
        ...
        velPub1 = rospy.Publisher(
            '/' + drone + '/joint1_velocity_controller/command', Float64, queue_size=4)
        velPub2 = rospy.Publisher(
            '/' + drone + '/joint2_velocity_controller/command', Float64, queue_size=4)
        velPub3 = rospy.Publisher(
            '/' + drone + '/joint3_velocity_controller/command', Float64, queue_size=4)
        velPub4 = rospy.Publisher(
            '/' + drone + '/joint4_velocity_controller/command', Float64, queue_size=4)
        ...
        PoseSub = rospy.Subscriber('/gazebo/model_states', ModelStates, control_esp,
                                   (velPub1, velPub2, velPub3, velPub4, err_rollPub, err_pitchPub, err_yawPub, vel))
